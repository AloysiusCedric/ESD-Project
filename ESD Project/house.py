...
@app.route("/confirmation" , methods=['POST']) # invoking data request from booking complex ms
def get_houseId():
    if request.is_json:
        order = request.get_json()
        result = processOrder(order)
        return jsonify(result), result["code"], print(order)
    else:
...
def processOrder(order):
    ...
    house_data = requests.get(f'http://127.0.0.1:5000/house/{houseId}').json() #return
